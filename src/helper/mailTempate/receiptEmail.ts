export interface IReceiptEmailData {
  totalAmount: number;
  amountInWord: string;
  allYear: string; // "2019,2027,2029,2023"
  title: string;
  name: string;
  idNo: string;
}

const formatYearsList = (years: string): string =>
  years
    .split(",")
    .map((y) => y.trim())
    .filter(Boolean)
...
export const buildReceiptEmailHtml = (data: IReceiptEmailData): string => {
...
const row = (label: string, value: string, isLast = false): string => `
  <tr>
